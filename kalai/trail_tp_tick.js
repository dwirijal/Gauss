...
function test(cd, look, sl, td) {
  ...
  for (let i = look + 1; i < cd.length; i++) {
    ...
    const tick = process.env.TICK === '1';
    const p = cd[i].close;
    const hi = cd[i].high, lo = cd[i].low;
    let side = null, entry = p, slc = p * (1 - sl / 100), tpl = p * (1 + sl / 100), trailed = false, qtyLeft = 1;
    if (tick) {
      if (hi >= hh * 0.999) { side = 'L'; entry = hh; }
      else if (lo <= ll * 1.001) { side = 'S'; entry = ll; }
      else continue;
      slc = entry * (1 - sl / 100); tpl = entry * (1 + sl / 100);
    } else {
