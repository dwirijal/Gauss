...
import { requestToId } from "./_id.js";
/**
...
/** Tracks listeners per subscription payload, resubscribes on reconnect, enforces server-side limits. */ export class WebSocketSubscriptionManager {
  /** Enable automatic re-subscription to Hyperliquid subscription after reconnection. */ resubscribe;
  _socket;
  _dispatcher;
  _hlEvents;
   ...
   */ async subscribe(channel, payload, listener, options) {
    ...
    if (signal?.aborted) {
      throw new WebSocketRequestError("Subscription was aborted", {
        cause: signal.reason,
        request: payload
      });
    }
  ...
  // Event handlers
  // ===========================================================================
  /** Resubscribes to every completed subscription when the socket re-opens. */ _handleOpen() {
    ...
    for (const [id, subscription] of [
      ...
      if (!this.resubscribe) {
        this._failSubscription(id, subscription, new WebSocketRequestError("WebSocket connection closed", {
   ...
   */ _handleClose() {
    ...
    for (const [id, subscription] of [
      ...
      const error = terminal ? new WebSocketRequestError("WebSocket connection permanently terminated", {
        cause: this._socket.terminationSignal.reason,
        request: JSON.parse(id)
      }) : new WebSocketRequestError("WebSocket connection closed", {
        request: JSON.parse(id)
      });
      this._failSubscription(id, subscription, error);
    }
  ...
  // ===========================================================================
  /** True when subscribing `payload` would track one user above the limit. */ _exceedsUserLimit(payload) {
    const userOf = (p)=>typeof p === "object" && p !== null && "user" in p && typeof p.user === "string" ? p.user.toLowerCase() : undefined;
