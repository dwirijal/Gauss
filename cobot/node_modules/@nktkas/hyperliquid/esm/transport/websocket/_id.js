 ...
 * @module
 */ /** Matches a `0x`-prefixed hex string. */ const HEX_REGEX = /^0x[0-9a-f]+$/i;
/**
 ...
 * ```
 */ export function requestToId(value) {
 ...
 * // => 3
 * ```
 */ export function specificity(value) {
  if (typeof value !== "object" || value === null) return 1;
  return Object.values(value).reduce((sum, item)=>sum + specificity(item), 0);
}
/**
 ...
 */ export function isSubset(subset, superset) {
  ...
  if (typeof subset === "string" && typeof superset === "string") {
    return HEX_REGEX.test(subset) && HEX_REGEX.test(superset) ? subset.toLowerCase() === superset.toLowerCase() : subset === superset;
  }
  ...
  if (Array.isArray(subset)) {
    return Array.isArray(superset) && subset.length === superset.length && subset.every((item, i)=>isSubset(item, superset[i]));
  }
...
/** Recursively sorts object keys and lowercases hex strings. */ function normalize(value) {
