 ...
 */ export const AgentSendAssetRequest = /* @__PURE__ */ (()=>{
  return v.object({
    /** Action to perform. */ action: v.object({
      /** Type of action. */ type: v.literal("agentSendAsset"),
      /** Destination address. */ destination: Address,
      /** Source DEX ("" for default USDC perp DEX, "spot" for spot). */ sourceDex: v.string(),
      /** Destination DEX ("" for default USDC perp DEX, "spot" for spot). */ destinationDex: v.string(),
      /** Token identifier. */ token: v.string(),
      /** Amount to send (not in wei). */ amount: UnsignedDecimal,
      /** Source sub-account address ("" for main account). */ fromSubAccount: v.optional(v.union([
        v.literal(""),
        Address
      ]), ""),
...
import { globalNonceManager } from "./_base/_nonce.js";
/** Schema for action fields (excludes request-level system fields). */ const AgentSendAssetActionSchema = /* @__PURE__ */ (()=>{
 ...
 *
 * @see https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/exchange-endpoint#agent-send-asset
 */ export function agentSendAsset(config, params, opts) {
  ...
  return executeL1Action({
    ...config,
    nonceManager: async (addr)=>// Patch action.nonce in-place so the body matches the envelope nonce (server requires equality).
      action.nonce = await (config.nonceManager?.(addr) ?? globalNonceManager.getNonce(`${addr}:${config.transport.isTestnet}`))
  }, action, opts);
}
