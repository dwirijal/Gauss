 ...
 */ export const CDepositRequest = /* @__PURE__ */ (()=>{
  return v.object({
    /** Action to perform. */ action: v.object({
      /** Type of action. */ type: v.literal("cDeposit"),
      /** Chain ID in hex format for EIP-712 signing. */ signatureChainId: Hex,
      /** Hyperliquid network type. */ hyperliquidChain: v.picklist([
        "Mainnet",
        "Testnet"
      ]),
      /** Amount of wei to deposit into staking balance (float * 1e8). */ wei: v.pipe(UnsignedInteger, v.minValue(1)),
...
/** Schema for action fields (excludes request-level system fields). */ const CDepositActionSchema = /* @__PURE__ */ (()=>{
  return v.omit(v.object(CDepositRequest.entries.action.entries), [
    "signatureChainId",
    "hyperliquidChain",
    "nonce"
  ]);
})();
/** EIP-712 types for the {@linkcode cDeposit} function. */ export const CDepositTypes = {
 ...
 */ export function cDeposit(config, params, opts) {
  const action = canonicalize(CDepositActionSchema, parse(CDepositActionSchema, {
    type: "cDeposit",
    ...params
  }));
