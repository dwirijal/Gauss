 ...
 */ export const CWithdrawRequest = /* @__PURE__ */ (()=>{
  return v.object({
    /** Action to perform. */ action: v.object({
      /** Type of action. */ type: v.literal("cWithdraw"),
      /** Chain ID in hex format for EIP-712 signing. */ signatureChainId: Hex,
      /** Hyperliquid network type. */ hyperliquidChain: v.picklist([
        "Mainnet",
        "Testnet"
      ]),
      /** Amount of wei to withdraw from staking balance (float * 1e8). */ wei: v.pipe(UnsignedInteger, v.minValue(1)),
...
/** Schema for action fields (excludes request-level system fields). */ const CWithdrawActionSchema = /* @__PURE__ */ (()=>{
  return v.omit(v.object(CWithdrawRequest.entries.action.entries), [
    "signatureChainId",
    "hyperliquidChain",
    "nonce"
  ]);
})();
/** EIP-712 types for the {@linkcode cWithdraw} function. */ export const CWithdrawTypes = {
 ...
 */ export function cWithdraw(config, params, opts) {
  const action = canonicalize(CWithdrawActionSchema, parse(CWithdrawActionSchema, {
    type: "cWithdraw",
    ...params
  }));
