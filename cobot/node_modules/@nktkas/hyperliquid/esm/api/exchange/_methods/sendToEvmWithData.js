 ...
 */ export const SendToEvmWithDataRequest = /* @__PURE__ */ (()=>{
  return v.object({
    /** Action to perform. */ action: v.object({
      /** Type of action. */ type: v.literal("sendToEvmWithData"),
      /** Chain ID in hex format for EIP-712 signing. */ signatureChainId: Hex,
      /** Hyperliquid network type. */ hyperliquidChain: v.picklist([
        "Mainnet",
        "Testnet"
      ]),
      /** Token identifier (e.g., "USDC"). */ token: v.string(),
      /** Amount to send (not in wei). */ amount: UnsignedDecimal,
      /** Source DEX name to transfer from. */ sourceDex: v.string(),
      /** Recipient address in the specified encoding format. */ destinationRecipient: v.string(),
      /** Address encoding format. */ addressEncoding: v.picklist([
        "hex",
        "base58"
      ]),
      ...
      /** Additional data payload (hex-encoded bytes, "0x" for empty). */ data: v.pipe(v.string(), v.regex(/^0[xX]([0-9a-fA-F]+)?$/)),
...
/** Schema for action fields (excludes request-level system fields). */ const SendToEvmWithDataActionSchema = /* @__PURE__ */ (()=>{
...
/** EIP-712 types for the {@linkcode sendToEvmWithData} function. */ export const SendToEvmWithDataTypes = {
 ...
 */ export function sendToEvmWithData(config, params, opts) {
  const action = canonicalize(SendToEvmWithDataActionSchema, parse(SendToEvmWithDataActionSchema, {
