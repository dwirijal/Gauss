...
/** Multi-sig config or `null` to revert to single-sig. */ const ConvertToMultiSigUserRequestSignersSchema = /* @__PURE__ */ (()=>{
  return v.nullable(/** Multi-signature configuration. */ v.object({
    /** List of authorized user addresses. */ authorizedUsers: v.array(Address),
    /** Minimum number of signatures required. */ threshold: v.pipe(UnsignedInteger, v.minValue(1), v.maxValue(10))
  }));
})();
/**
 ...
 */ export const ConvertToMultiSigUserRequest = /* @__PURE__ */ (()=>{
  return v.object({
    /** Action to perform. */ action: v.object({
      /** Type of action. */ type: v.literal("convertToMultiSigUser"),
      /** Chain ID in hex format for EIP-712 signing. */ signatureChainId: Hex,
      /** Hyperliquid network type. */ hyperliquidChain: v.picklist([
        "Mainnet",
        "Testnet"
      ]),
      /**
...
import { executeUserSignedAction } from "./_base/mod.js";
/** Schema for action fields (excludes request-level system fields). */ const ConvertToMultiSigUserActionSchema = /* @__PURE__ */ (()=>{
...
/** EIP-712 types for the {@linkcode convertToMultiSigUser} function. */ export const ConvertToMultiSigUserTypes = {
 ...
 */ export function convertToMultiSigUser(config, params, opts) {
  const action = canonicalize(ConvertToMultiSigUserActionSchema, parse(ConvertToMultiSigUserActionSchema, {
